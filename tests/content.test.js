...
assert.match(privacy, /Resend delivers account verification and password-recovery emails/);
assert.match(privacy, /Deleting an account removes its Sudoku sync data and Neon Auth user/);
assert.doesNotMatch(privacy, /does not provide an account/);
assert.match(privacy, /Screenshot OCR uses an online provider/);
assert.match(privacy, /Sudoku OCR through RapidAPI/);
assert.match(privacy, /retention policy is not publicly detailed/);
